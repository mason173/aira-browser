...
Amended 2026-07-20: some providers return `409 Conflict` rather than `404 Not Found` when the first snapshot read targets a path whose intermediate collections do not exist. On that first-read conflict, the App and Airatab WebDAV adapters each create their fixed root collections in order and retry the same GET once; only a resulting `404` means that the current-generation remote is empty, while a persistent `409` or any other error remains fail-closed.
...
Amended 2026-08-04 after App and Airatab both reported an invalid Aira Cloud snapshot: ordinary three-way merge still
...
Airatab's ordinary and missing-baseline merges. It does not suppress an ordinary same-type deletion, revive an entity
...
Airatab: every folder/item parent and order parent exists, every live entity appears exactly once in the order matching
...
the one target write, successful local apply, and target-identity baseline commit path. Airatab, Additional Backup,
...
Amended 2026-08-04 after the cross-client generation audit: Airatab uses the same `/sync/v3/bookmarks` Aira Cloud state,
...
execution lock are retained. Airatab confirms local materialization before saving its provider baseline; because its
...
to the desktop browser tree. Superseding the earlier App-only scope, an established Airatab switch between Aira Cloud and
...
Amended 2026-08-04 after an Airatab first-sync preservation duplicate was deleted as a complete folder subtree: the
Airatab merge result discards every non-root order whose parent is not a folder in the merged live entity set. Historical
...
shared by Airatab Aira Cloud and WebDAV; it does not change first-sync preservation, order conflict selection, tombstone
